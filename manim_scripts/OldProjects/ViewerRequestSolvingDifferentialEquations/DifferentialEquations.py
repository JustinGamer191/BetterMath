...
class Second(Scene):
    def construct(self):
        ...
        step4Yap.scale(0.8)
        step4YapCont = MathTex(
            r"\text{in the form of } \frac{u'}{1 + u^2} \text{ to integrate using arctan.}"
        )
...
class Third(Scene):
    def construct(self):
        ...
        textTop.color = GOLD
        textNext = MathTex(
            r"\frac{\text{arctan} (\frac{3}{2} y)}{6} = - \frac{1}{2} e^{-2x-1}"
        )
        textNext.color = RED
        # Here is where the math went wrong.
        # It should be e^(2x+1).
